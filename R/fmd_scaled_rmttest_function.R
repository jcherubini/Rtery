#' Allometric scaling for flow-mediated dilation: Repeated measures t-tests
#'
#' This function calculates allometrically-scaled flow-mediated dilation responses for repeated measures t-test study designs, and returns  comparisons between groups. The function will also return back-transformed means, standard errors, and 95% confidence intervals. You need to label the columns in your data set as "dpeak", "dbase", and "group" respectively.
...
#'       \item{plot}{Graphical representation using `ggplot2` syntax}
...
#' dat <- data.frame(pid = as.factor(rep(1:10, 2)), dpeak = rnorm(20, 4.27, 1.12), dbase = rnorm(20, 4.16, 1.21)*0.8, group = as.factor(c(rep("NS", 10), rep("SD", 10))))
#' dat <- dat |>
#' dplyr::arrange(pid)
#' res <- fmd_scaled_rmttest(dat)
#'
#' @import dplyr
#' @import lmerTest
#' @import emmeans
...
fmd_scaled_rmttest <- function(dat){
  ...
  if(fit$coefficients[2] != 1 & confint(fit, level = 0.95)[2,2] < 1){
    ...
    fit2 <- lmer(difflogs ~ log_dbase + group + (1|pid), data = newdat)
    lmcoef.aov <- anova(fit2, type = "III", ddf = "Kenward-Roger")
    em.means <- emmeans(fit2, "group")
    ...
    tf.em.means <- data.frame("Group" = (as.data.frame(em.means)[1]),
                              "Estimated Marginal Means" = ((exp(as.data.frame(em.means)[2])-1)*100),
                              "Std. Error" = ((exp(as.data.frame(em.means)[3])-1)*100),
                              "LL" = ((exp(as.data.frame(em.means)[5])-1)*100),
                              "UL" = ((exp(as.data.frame(em.means)[6])-1)*100))
    ...
    # Create a table for transformed estimated marginal means stats...
    tf.stats <- data.frame("contrast" = (as.data.frame(em.means.cont.p)[,1]),
                           "std.error" = ((exp(as.data.frame(em.means.cont.p)[,3])-1)*100),
                           "df" = (as.data.frame(em.means.cont.p)[,4]),
                           "t.stat" = (as.data.frame(em.means.cont.p)[,5]),
                           "p-value" = (as.data.frame(em.means.cont.p)[,6]),
                           "lower.95CI" = ((exp(as.data.frame(em.means.cont.ci)[,5])-1)*100),
                           "upper.95CI" = ((exp(as.data.frame(em.means.cont.ci)[,6])-1)*100))

    # Plot
    plot <- ggplot(tf.em.means, aes(x = Group, y = Est.MarginalMeans, group = 1)) +
    ...
    attr(value, "class") <- "fmd_scaled_rmttest"
    value
